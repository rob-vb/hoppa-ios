...
`79 kg + 1 + 1` still adds and `57 kg` is not a pin on 125 lbs (`pin at 56.7 kg · 2.5 kg`).
...
Per side: 20 + 1.25          21.25 kg per side
```
...
`… · BASE 15 KG`. The load line prefixes a slightly smaller `Per side:` onto the plates
(`20 + 5 + 2.5 + 1.25`); the qualifier names the Base Weight with the per-side sum
(`15 base + 28.75 kg per side`). Mixing the base into the plate list made it unclear
...
`5 kg` is not a pin on that stack. It is `pin at 4.5 kg · 0.5 kg`. `57 kg` is not a pin on 125 lbs.
It is `pin at 56.7 kg · 2.5 kg`.
